...
final class UpdateCommentUsecaseTest extends TestCase
{
    ...
    {
        $container = new DI\Container();
        $updateCommentUsecase = $container->get(UpdateCommentUsecase::class);

        $commentPayload = new UpdateCommentDTO(body: '');

        $this->expectException(Exception::class);
        $this->expectExceptionMessage('The comment cannot be empty');
        $this->expectExceptionCode(400);

        $updateCommentUsecase->execute(self::$comment->getId(), $commentPayload);
    }
    ...
    public function testThrowsANotFoundCommentException(): void
    {
        ...
        $commentPayload = new UpdateCommentDTO(
            body: 'This is my new comment!',
        );

        $this->expectException(Exception::class);
        $this->expectExceptionMessage('Comment cannot be found');
        $this->expectExceptionCode(404);

        $updateCommentUsecase->execute(self::$comment->getId() + 1, $commentPayload);
    }

    public function testShouldUpdateOneCard(): void
    {
        $container = new DI\Container();
